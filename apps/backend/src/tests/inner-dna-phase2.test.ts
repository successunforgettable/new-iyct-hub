...
function simulateHeroMoments(
  ...
  rhetiGap: number = 5, // Default to clear (>= 3 means RHETI was clear)
  ...
  // RHETI clarity flags (matches service logic)
  ...
  for (let i = 0; i < maxScenarios && !isComplete; i++) {
    ...
    // Check completion logic (MATCHES SERVICE - with RHETI gap awareness)
    const sorted = Object.entries(typeCounts).sort((a, b) => b[1] - a[1]);
    const dominantType = parseInt(sorted[0][0]);
    const dominantCount = sorted[0][1];
    const secondCount = sorted[1] ? sorted[1][1] : 0;
    const heroGap = dominantCount - secondCount;
    const heroMatchesRheti = dominantType === topTypes[0];

    // SMART COMPLETION - with RHETI gap awareness (matches service)
    if (isRhetiClear && heroMatchesRheti && dominantCount >= 2) {
      // BEST CASE: RHETI was clear and Hero confirms
      isComplete = true;
    } else if (!isRhetiClose && dominantCount >= 3 && heroGap >= 2) {
      // RHETI was clear, Hero has decisive winner
      isComplete = true;
    } else if (!isRhetiClose && dominantCount >= 3 && heroGap >= 1) {
      // RHETI was clear, Hero has slight winner
      isComplete = true;
...
function runHeroSimulation(
  ...
  runs: number = 100,
  rhetiGap: number = 5 // Default to clear race
): {
  winRate: number;
  avgScenarios: number;
  ...
  for (let i = 0; i < runs; i++) {
    const result = simulateHeroMoments(topTypes, pattern, rhetiGap);
...
function testHeroMomentsNarrowing() {
  ...
  // rhetiGap indicates how clear RHETI was (< 3 = close, >= 3 = clear)
  const scenarios: Array<{ topTypes: number[]; pattern: AnswerPattern; expected: number; desc: string; rhetiGap: number }> = [
    // Clear winner patterns (RHETI was clear, gap >= 3)
    { topTypes: [8, 3, 7], pattern: { 8: 70, 3: 20, 7: 10 }, expected: 8, desc: 'Clear Type 8 (70%)', rhetiGap: 5 },
    { topTypes: [3, 8, 7], pattern: { 3: 65, 8: 25, 7: 10 }, expected: 3, desc: 'Clear Type 3 (65%)', rhetiGap: 4 },
    { topTypes: [1, 6, 2], pattern: { 1: 60, 6: 30, 2: 10 }, expected: 1, desc: 'Clear Type 1 (60%)', rhetiGap: 3 },
    
    // Moderate winner patterns (RHETI somewhat clear)
    { topTypes: [8, 3, 7], pattern: { 8: 50, 3: 35, 7: 15 }, expected: 8, desc: 'Moderate Type 8 (50%)', rhetiGap: 3 },
    { topTypes: [5, 9, 4], pattern: { 5: 50, 9: 35, 4: 15 }, expected: 5, desc: 'Moderate Type 5 (50%)', rhetiGap: 3 },
    
    // Close race patterns (RHETI was CLOSE, gap < 3) - requires more scenarios
    { topTypes: [8, 3, 7], pattern: { 8: 45, 3: 40, 7: 15 }, expected: 8, desc: 'Close Type 8 vs 3 (45/40)', rhetiGap: 1 },
    { topTypes: [1, 6, 2], pattern: { 1: 45, 6: 42, 2: 13 }, expected: 1, desc: 'Close Type 1 vs 6 (45/42)', rhetiGap: 1 },
    
    // Correct type is #2 in RHETI but should still win Hero
    { topTypes: [3, 8, 7], pattern: { 8: 55, 3: 30, 7: 15 }, expected: 8, desc: 'Type 8 was #2 in RHETI but wins Hero', rhetiGap: 2 },
  ];

  scenarios.forEach(s => {
    const result = runHeroSimulation(s.topTypes, s.pattern, s.expected, 200, s.rhetiGap);
    
    test(
      'Hero Narrowing',
      `${s.desc} → Wins: ${(result.winRate * 100).toFixed(0)}%`,
      result.winRate >= 0.50, // Should win more than half the time
  ...
  for (let targetType = 1; targetType <= 9; targetType++) {
    ...
    for (let i = 0; i < runs; i++) {
      ...
      if (inTop3) {
        ...
        // Use actual RHETI gap from simulation
  ...
  confusablePairs.forEach(pair => {
    ...
    for (let i = 0; i < runs; i++) {
      ...
      if (rhetiResult.topTypes.includes(pair.a)) {
        // Simulate Hero with same pattern, using actual RHETI gap
        const heroPattern: AnswerPattern = { 
...
function testEdgeCases() {
  ...
  // Edge Case 2: Hero Moments with very close race (RHETI was close)
  console.log('\n  [Very Close Hero Race]\n');
  
  const closeRacePattern: AnswerPattern = { 8: 34, 3: 33, 7: 33 };
  const closeRace = runHeroSimulation([8, 3, 7], closeRacePattern, 8, 200, 1); // rhetiGap = 1 (close)
  test(
    'Edge Case',
